#!/usr/bin/env python
#D. Jones - 1/17/14
"""This code is from the IDL Astronomy Users Library, converted by D. Jones for PythonPhot"""
...
def find(image,
    ...
    # SEARCH:
    #Threshold dependent search begins here
    ...
    return np.array(zip(x[0:nstar+1], y[0:nstar+1], flux[0:nstar+1],
                        sharp [0:nstar+1], roundness[0:nstar+1]),
                    dtype=np.dtype([('x','float64'), ('y','float64'), ('flux','float64'),
                                    ('sharpness','float64'), ('roundness','float64')]))
